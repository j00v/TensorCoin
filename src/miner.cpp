...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to ztnsrs that have been in mempool long
                    //and higher priority to ztnsrs that are large in value
                    ...
                    // zTNSR spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zTNSR spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zTNSR serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
		...
		uint256 hashBlockLastAccumulated = (heightOfBlockLastAccumulated > 0)
			...
			//For the period before v2 activation, zEPM will be disabled and previous block's checkpoint is all that will be needed
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                //Sign block with the zTNSR key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with zTNSR key failed \n", __func__);
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("Tensor CoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            pwalletMain->ztnsrTracker->RemovePending(pblock->vtx[1].GetHash());
            pwalletMain->ztnsrTracker->ListMints(true, true, true); //update the state
        }
        return error("Tensor CoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("Tensor CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tensorcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running Tensor CoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("Tensor CoinMiner exception");
    } catch (...) {
        LogPrintf("Tensor CoinMiner exception");
    }

    LogPrintf("Tensor CoinMiner exiting\n");
}
