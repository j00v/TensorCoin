...
CzTNSRWallet::CzTNSRWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing ztnsr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTNSRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTNSRSeed_deprecated()) {
                    LogPrintf("%s: Updated zTNSR seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztnsr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztnsr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzTNSRWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTNSRCount(nCountLastUsed);
    else if (!walletdb.ReadZTNSRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTNSRWallet::Lock()
{
    seedMaster = 0;
}

void CzTNSRWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTNSRWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTNSR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTNSRWallet::LoadMintPoolFromDB()
{
...
void CzTNSRWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTNSRWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTNSRWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztnsrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTNSRWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTNSR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztnsrTracker which also adds to database
    pwalletMain->ztnsrTracker->Add(dMint, true);
...
void CzTNSRWallet::SeedToZTNSR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTNSRWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTNSRWallet::GenerateDeterministicZTNSR(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTNSRWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTNSRWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzTNSRWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
