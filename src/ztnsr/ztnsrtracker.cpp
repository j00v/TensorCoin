...
CzTNSRTracker::CzTNSRTracker(std::string strWalletFile)
{
...
CzTNSRTracker::~CzTNSRTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzTNSRTracker::Init()
{
...
bool CzTNSRTracker::Archive(CMintMeta& meta)
{
...
bool CzTNSRTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzTNSRTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzTNSRTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzTNSRTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzTNSRTracker::GetSpendCache(const uint256& hashStake)
{
...
std::vector<uint256> CzTNSRTracker::GetSerialHashes()
{
...
CAmount CzTNSRTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzTNSRTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

//Does a mint in the tracker have this txid
bool CzTNSRTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzTNSRTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzTNSRTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzTNSRTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzTNSRTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzTNSRTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzTNSRTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzTNSRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzTNSRWallet* zTNSRWallet)
{
    bool iszTNSRWalletInitialized = (NULL != zTNSRWallet);
    CMintMeta meta;
    ...
    if (! iszTNSRWalletInitialized)
        zTNSRWallet = new CzTNSRWallet(strWalletFile);
    meta.isSeedCorrect = zTNSRWallet->CheckSeed(dMint);
    if (! iszTNSRWalletInitialized)
        delete zTNSRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzTNSRTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzTNSRTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzTNSRTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzTNSRTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzTNSRTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzTNSRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzTNSRWallet* zTNSRWallet = new CzTNSRWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zTNSRWallet);
        }
        delete zTNSRWallet;
        LogPrint("zero", "%s: added %d dztnsr from DB\n", __func__, listDeterministicDB.size());
    }
