...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::TNSR, nValue);
...
{
    std::string genesisblockhash = "0000059b5bfa4d46f2fe779ef269feafe9114541c64deb7bd8440d85a6df4c04";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (tensorcoin.conf).");
            QMessageBox::warning(this, "Tensor Coin Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
