...
#include "qt/tensorcoin/loadingdialog.h"
                                                                                                            ...
                                                                                                            btnWatch(new QCheckBox())
{
    ...
    case Mode::Encrypt: // Ask passphrase x2
        ui->warningLabel->setText(tr("We have decided to make obligatory wallet encryption in order to increase security level.<br/>Enter the new passphrase to the wallet.<br/>Please use a passphrase of <b>ten or more random characters</b>, or <b>eight or more words</b>."));
        ui->passLabel1->hide();
        ui->passEdit1->hide();
        ui->layoutEdit->hide();
...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                tr("Warning: If you encrypt your wallet and lose your passphrase, you will <b>LOSE ALL OF YOUR TNSR</b>!") + "<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                TensorCoinGUI* window = static_cast<TensorCoinGUI*>(parentWidget());
...
bool AskPassphraseDialog::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn){
    TensorCoinGUI* gui = static_cast<TensorCoinGUI*>(parentWidget());
...
void AskPassphraseDialog::warningMessage() {
    hide();
    static_cast<TensorCoinGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("Your wallet will be restarted to finish the encryption process. "
               "Remember that encrypting your wallet cannot fully protect "
               "your TNSRs from being stolen by malware infecting your computer.") +
            "<br><br><b>" +
    ...
    static_cast<TensorCoinGUI*>(parentWidget())->handleRestart(args);
}
