                                                ...
                                                fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zTNSR ought to be enough for anybody." - Bill Gates, 2017
    ui->zTNSRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTNSRSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_pushButtonSpendzTNSR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTNSR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zTNSR, true));
        if (!ctx.isValid()) {
...
void PrivacyDialog::on_pushButtonZTNSRControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZTNSRControlDialog* zTNSRControl = new ZTNSRControlDialog(this);
    zTNSRControl->setModel(walletModel);
    zTNSRControl->exec();
}

void PrivacyDialog::setZTNSRControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTNSRSelected_int->setText(QString::number(nAmount));
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTNSR </b>";
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zTNSR </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zTNSR </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzTNSRSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzTNSR->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zTNSR
        //ui->pushButtonMintzTNSR->setEnabled(false);
        //ui->pushButtonMintzTNSR->setToolTip(tr("zTNSR is currently disabled due to maintenance."));

        // Spend zTNSR
        ui->pushButtonSpendzTNSR->setEnabled(false);
        ui->pushButtonSpendzTNSR->setToolTip(tr("zTNSR is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zTNSR
        //ui->pushButtonMintzTNSR->setEnabled(true);
        //ui->pushButtonMintzTNSR->setToolTip(tr("PrivacyDialog", "Enter an amount of TNSR to convert to zTNSR", 0));

        // Spend zTNSR
        ui->pushButtonSpendzTNSR->setEnabled(true);
        ui->pushButtonSpendzTNSR->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
