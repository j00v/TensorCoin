...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::TNSR);
    ...
    if (settings.contains("nAnonymizeTensorCoinAmount") || reset)
        SoftSetArg("-anonymizetensorcoinamount", settings.value("nAnonymizeTensorCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tensorcoin.cpp during shotdown to also remove the window positions
