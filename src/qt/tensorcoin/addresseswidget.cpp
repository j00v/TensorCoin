...
#include "qt/tensorcoin/forms/ui_addresseswidget.h"
#include "qt/tensorcoin/addresslabelrow.h"
#include "qt/tensorcoin/addnewaddressdialog.h"
#include "qt/tensorcoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(TensorCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a Tensor Coin address"));
    setCssProperty(ui->labelAddress, "text-title");
    ui->lineEditAddress->setPlaceholderText("e.g. R7VFR83SQbiezrW72hjc…");
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress tnsrAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(tnsrAdd.Get(), label.toUtf8().constData(), "send")) {
