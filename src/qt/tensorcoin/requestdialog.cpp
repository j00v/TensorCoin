...
#include "qt/tensorcoin/requestdialog.h"
#include "qt/tensorcoin/forms/ui_requestdialog.h"
#include <QListView>
    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of share a Tensor Coin address, you can create a Payment Request message which bundles up more information than is contained in just a Tensor Coin address."));
    ...
    ui->comboBoxCoin->addItem("TNSR", 0);
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 TNSR");
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        ui->labelTitle->setText("Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " TNSR");
