...
#include "qt/tensorcoin/settings/forms/ui_settingswidget.h"
...
SettingsWidget::SettingsWidget(TensorCoinGUI* parent) :
    PWidget(parent),
...
void SettingsWidget::onSaveOptionsClicked(){
    if(mapper->submit()) {
        ...
        if (this->clientModel->getOptionsModel()->isRestartRequired()) {
            openStandardDialog(tr("Restart required"), tr("Your wallet will be restarted to apply the changes\n"), tr("OK"));
