...
#include "qt/tensorcoin/topbar.h"
#include "qt/tensorcoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/tensorcoin/lockunlock.h"
#include "qt/tensorcoin/qtutils.h"
#include "qt/tensorcoin/receivedialog.h"
...
TopBar::TopBar(TensorCoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopTNSR}, "amount-small-topbar");
    setCssProperty({ui->labelAmountTNSR}, "amount-topbar");
    setCssProperty({ui->labelPendingTNSR, ui->labelImmatureTNSR}, "amount-small-topbar");
...
void TopBar::triggerEncryptWhileUnencrypted() {
    if (!walletModel) {
        return;
    }
    if (isLightTheme())
    ...
    while (walletModel->getEncryptionStatus() == WalletModel::Unencrypted) {
        encryptWallet();
    }
}
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("TensorCoin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount tnsrAvailableBalance = balance - nLockedBalance - immatureBalance; // TODO_TNSR locked on pos == 0

    // zTNSR Balance

    // Set
    QString totalTNSR = GUIUtil::formatBalance(tnsrAvailableBalance, nDisplayUnit);
    // Top
    ui->labelAmountTopTNSR->setText(totalTNSR);
    

    // Expanded
    ui->labelAmountTNSR->setText(totalTNSR);
    

    ui->labelPendingTNSR->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));

    ui->labelImmatureTNSR->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
}
