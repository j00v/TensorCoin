...
#include "qt/tensorcoin/dashboardwidget.h"
#include "qt/tensorcoin/forms/ui_dashboardwidget.h"
#include "qt/tensorcoin/sendconfirmdialog.h"
#include "qt/tensorcoin/txrow.h"
...
DashboardWidget::DashboardWidget(TensorCoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	// Staking Information
	ui->labelMessage->setText(tr("Amount of TNSR staked."));
	setCssSubtitleScreen(ui->labelMessage);
	setCssProperty(ui->labelSquareTNSR, "square-chart-tnsr");
	setCssProperty(ui->labelTNSR, "text-chart-tnsr");
	...
	ui->labelAmountTNSR->setText("0 TNSR");
	setCssProperty(ui->labelAmountTNSR, "text-stake-tnsr-disable");
	...
	ui->btnHowTo->setText(tr("How to get TNSR"));
...
void DashboardWidget::loadChartData(bool withMonthNames) {
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			tnsr = (pair.first != 0) ? pair.first / 100000000 : 0;

			chartData->totalTNSR += pair.first;

		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesTNSR.append(tnsr);


		int max = std::max(tnsr, tnsr);
...
void DashboardWidget::onChartRefreshed() {
	...
	// init sets
	set0 = new QBarSet("TNSR");
	set0->setColor(QColor(78, 98, 129));
	...
	set0->append(chartData->valuesTNSR);


	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalTNSR > 0) {
		setCssProperty(ui->labelAmountTNSR, "text-stake-tnsr");
	}
	else {
		setCssProperty(ui->labelAmountTNSR, "text-stake-tnsr-disable");
	}
	forceUpdateStyle({ ui->labelAmountTNSR });
	ui->labelAmountTNSR->setText(GUIUtil::formatBalance(chartData->totalTNSR, nDisplayUnit));
