...
#include "qt/tensorcoin/forms/ui_send.h"
#include "qt/tensorcoin/addnewcontactdialog.h"
#include "qt/tensorcoin/qtutils.h"
#include "qt/tensorcoin/sendchangeaddressdialog.h"
#include "qt/tensorcoin/optionbutton.h"
#include "qt/tensorcoin/sendconfirmdialog.h"
#include "qt/tensorcoin/myaddressrow.h"
...
SendWidget::SendWidget(TensorCoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::send),
	...
	btnContacts(new QPushButton())
{
	...
	/* Subtitle */
	ui->labelSubtitle1->setText(tr("You can transfer public coins (TNSR)"));
	setCssProperty(ui->labelSubtitle1, "text-subtitle");

	/* Address */
	ui->labelSubtitleAddress->setText(tr("Enter a Tensor Coin address or contact label"));
	...
	ui->labelAmountSend->setText("0.00 TNSR");
	setCssProperty(ui->labelAmountSend, "text-body1");

	//send button
	ui->pushButtonSave->setText(tr("Send TNSR"));
	...
	setCssProperty(coinIcon, "coin-icon-tnsr");
...
void SendWidget::onSendClicked() {
	...
	// will call relock
	if (!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Send_TNSR, true)) {
...
void SendWidget::onCoinControlClicked() {
	if (walletModel->getBalance() > 0) {
		...
		inform(tr("You don't have any TNSR to select."));
	}
}
...
void SendWidget::onContactMultiClicked() {
	if (focusedEntry) {
		...
		CBitcoinAddress tnsrAdd = CBitcoinAddress(address.toStdString());
		if (walletModel->isMine(tnsrAdd)) {
		...
		if (dialog->res) {
			...
			if (walletModel->updateAddressBookLabels(tnsrAdd.Get(), dialog->getLabel().toStdString(), "send")) {
