...
#include "ui_ztnsrcontroldialog.h"
...
std::set<std::string> ZTNSRControlDialog::setSelectedMints;
std::set<CMintMeta> ZTNSRControlDialog::setMints;

bool CZTNSRControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZTNSRControlDialog::COLUMN_DENOMINATION || column == ZTNSRControlDialog::COLUMN_VERSION || column == ZTNSRControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZTNSRControlDialog::ZTNSRControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZTNSRControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zTNSR Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZTNSR->setProperty("cssClass", "text-main-purple");
    ui->labelZTNSR_int->setProperty("cssClass", "text-main-purple");
...
ZTNSRControlDialog::~ZTNSRControlDialog()
{
    delete ui;
}

void ZTNSRControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTNSRControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZTNSRControlWidgetItem* itemDenom(new CZTNSRControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZTNSRControlWidgetItem *itemMint = new CZTNSRControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->ztnsrTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->ztnsrTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zTNSR.";
            else if (!mint.isSeedCorrect)
                strReason = "The zTNSR seed used to mint this zTNSR is not the same as currently hold in the wallet";
            else
...
void ZTNSRControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTNSRControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZTNSR_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZTNSRControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZTNSRControlDialog::GetSelectedMints()
{
...
void ZTNSRControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
