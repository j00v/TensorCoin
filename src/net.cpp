...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (TensorCoin users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "Tensor Coin " + FormatFullVersion();
...
void ThreadOpenAddedConnections()
{
    {
        LOCK(cs_vAddedNodes);
        mapMultiArgs["-addnode"].push_back("161.35.74.34");
		mapMultiArgs["-addnode"].push_back("167.172.102.211");
		mapMultiArgs["-addnode"].push_back("161.35.31.8");
		mapMultiArgs["-addnode"].push_back("104.248.138.198");

        mapMultiArgs["-addnode"].push_back("167.172.100.241");
		mapMultiArgs["-addnode"].push_back("161.35.24.23");
		mapMultiArgs["-addnode"].push_back("161.35.31.249");

		mapMultiArgs["-addnode"].push_back("161.35.72.192");
        mapMultiArgs["-addnode"].push_back("142.93.171.107");
        mapMultiArgs["-addnode"].push_back("134.122.73.38");
...
bool BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Tensor Coin is probably already running."), addrBind.ToString());
        else
