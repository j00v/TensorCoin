...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue ztnsrObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ztnsrObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    ztnsrObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zTNSRsupply", ztnsrObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZTNSR()) ? "Not available" : std::to_string(
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"zTNSRsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zTNSR denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zTNSR denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zTNSR denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zTNSR denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zTNSR denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zTNSR denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zTNSR denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zTNSR denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zTNSR denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in tensor coin\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of tensor coin addresses\n"
            "     \"tensor coinaddress\"   	 	(string) tensorcoin address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (txin.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (isPublicSpend) {
                        ...
                        if (!ZTNSRModule::parseCoinSpend(txin, tx, prevOut, publicSpend)) {
...
UniValue getblockindexstats(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
                ...
                "  }\n"
                "  \"txbytes\": xxxxx                (numeric) Sum of the size of all txes (zTNSR excluded) over block range\n"
                "  \"ttlfee\": xxxxx                 (numeric) Sum of the fee amount of all txes (zTNSR mints excluded) over block range\n"
                "  \"ttlfee_all\": xxxxx             (numeric) Sum of the fee amount of all txes (zTNSR mints included) over block range\n"
