...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that ztnsrspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TNSR at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTNSR send
enum ZerocoinSpendStatus {
    ZTNSR_SPEND_OKAY = 0,                            // No error
    ZTNSR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTNSR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTNSR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTNSR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTNSR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTNSR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTNSR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTNSR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTNSR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTNSR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTNSR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTNSR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTNSR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTNSR_SPENT_USED_ZTNSR = 14,                      // Coin has already been spend
    ZTNSR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZTNSR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    static const int STAKE_SPLIT_THRESHOLD = 3;
    ...
    std::string GetUniqueWalletBackupName(bool fztnsrAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzTNSRTracker> ztnsrTracker;
    ...
    void setZWallet(CzTNSRWallet* zwallet)
    {
        zwalletMain = zwallet;
        ztnsrTracker = std::unique_ptr<CzTNSRTracker>(new CzTNSRTracker(strWalletFile));
    }

    CzTNSRWallet* getZWallet() { return zwalletMain; }
    ...
    /** zTNSR reset */
    boost::signals2::signal<void()> NotifyzTNSRReset;
