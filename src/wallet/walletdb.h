...
public:
    ...
    bool WriteZTNSRSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZTNSRSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZTNSRSeed_deprecated(uint256& seed);
    bool EraseZTNSRSeed();
    bool EraseZTNSRSeed_deprecated();

    bool WriteZTNSRCount(const uint32_t& nCount);
    bool ReadZTNSRCount(uint32_t& nCount);
